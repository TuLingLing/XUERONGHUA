...
public  class SysDepartmentServiceImpl: ISysDepartmentService
	...
	public ITSysDepartmentDao  SysDepartmentDao{set;get;}
        ...
        public IEnumerable<TSysDepartment> GetSysDepartmentList( int pageIndex, int pageSize, out int recordCount,
            Expression<Func<TSysDepartment, bool>> predicate,IDictionary<string, Ed.Entity.EdEnums.OrderEmum> orders)
        {
            try
            {
                var models = SysDepartmentDao.GetListByPage(pageIndex, pageSize, out recordCount, orders,
                    predicate);
			...
			try
                {
					...
					//这里添加参数
                    var models = SysDepartmentDao.GetListBySql(strSql.ToString(), namedParameters);
      ...
      public  List<Dictionary<string, object>>  GetSysDepartmentList(int pageIndex, int pageSize, string selector,string predicate, string ordering,
		 ...
		 try
                {
                var models = SysDepartmentDao.GetListByPage(pageIndex, pageSize, selector, predicate,
	   ...
	   try
                {
                var models = SysDepartmentDao.GetListWithExpByPage(pageIndex, pageSize, selector, expFields, expSelector, predicate,
                ordering,out recordCount, values);
				   LogHelper.Debug("获取SysDepartment分页列表成功。");
                return models;
                }
       ...
       public TSysDepartment GetSysDepartment(Expression<Func<TSysDepartment, bool>> predicate)
	   {
	    try
	   	      {
	   	          var model =  SysDepartmentDao.GetEntity(predicate);
	   	          return model;
	   	      }
	   	      catch (Exception ex)
	   	      {
	...
	try
            {
                var fields =  SysDepartmentDao.GetFields(selector, predicate);
                return fields;
            }
            catch (Exception ex)
            {
      ...
      public  Dictionary<string, object> GetSysDepartment(string selector, string expFields, string expSelector, string predicate,
            params object[] values){
			 try
                {
			  var model = SysDepartmentDao.GetEntityWithExp(selector, expFields, expSelector, predicate, values);
			   return model;
			    }
                catch (Exception ex)
                {
	  ...
	  #region 添加、修改
        ...
        public  bool AddSysDepartment(TSysDepartment SysDepartment)
		{
		 var status = false;
		       try
                {
                     status = SysDepartmentDao.InsertEntity(SysDepartment);
			    }
                catch (Exception ex)
                {
                    LogHelper.Error("获取SysDepartment失败。", ex);
        ...
        public bool EditSysDepartment(TSysDepartment SysDepartment)
		{
		 ...
		 var status = false;
		       try
                {
                     status = SysDepartmentDao.UpdateEntity(SysDepartment);
			    }
                catch (Exception ex)
                {
                    LogHelper.Error("获取SysDepartment失败。", ex);
       ...
       public bool UpdateEntityStatus(List<Field> fields, params  object[] values)
			{
            return  SysDepartmentDao.UpdateEntityStatus(fields, values);
			}
		 #endregion
        ...
        public bool DeleteSysDepartment(string ids)
		{
		       ...
		       try
                ...
                status = SysDepartmentDao.DeletesEntity(predicates);
				}
                catch (Exception ex)
                {
                    LogHelper.Error("获取SysDepartment失败。", ex);
        ...
        {
            var status = SysDepartmentDao.ExistsEntity(predicate,values);
            return status;
        }
