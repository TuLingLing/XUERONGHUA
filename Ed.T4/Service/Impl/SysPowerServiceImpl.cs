...
public  class SysPowerServiceImpl: ISysPowerService
	...
	public ITSysPowerDao  SysPowerDao{set;get;}
      ...
      public  List<Dictionary<string, object>>  GetSysPowerList(int pageIndex, int pageSize, string selector,string predicate, string ordering,
	   ...
	   try
                {
                var models = SysPowerDao.GetListWithExpByPage(pageIndex, pageSize, selector, expFields, expSelector, predicate,
                ordering,out recordCount, values);
				   LogHelper.Debug("获取SysPower分页列表成功。");
                return models;
                }
	...
	try
	  ...
	  #region 添加、修改
       ...
       public bool UpdateEntityStatus(List<Field> fields, params  object[] values)
			{
            return  SysPowerDao.UpdateEntityStatus(fields, values);
			}
		 #endregion
        ...
        {
            var status = SysPowerDao.ExistsEntity(predicate,values);
            return status;
        }
