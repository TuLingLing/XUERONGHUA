...
namespace  Ed.Dao.Impl
{
    ...
    /// <summary>
    /// 自动生成的实现ITSysModuleDao接口的Dao类。 2014-10-28 10:57:52 By 唐有炜
    /// </summary>
 public class TSysModuleDaoImpl:ITSysModuleDao
    {
	  ...
	  #region 读操作
	   ...
	   /// <summary>
        ...
        /// <returns>返回所有数据总数</returns>
       public int GetCount(Expression<Func<TSysModule, bool>> predicate)
        {
             using (EdDBContext db=new EdDBContext())
            {
             var models= db.TSysModules.Where<TSysModule>(predicate);
			 var sqlText = models.GetProperty("SqlText");
             LogHelper.Debug(sqlText.ToString());
			 return models.Count();
            }
        ...
        public List<TSysModule> GetList() 
        {
          using (EdDBContext db=new EdDBContext())
            {
             var models= db.TSysModules;
        ...
        /// <param name="predicate">Lamda表达式</param>
        /// <returns>返回所有数据列表</returns>
       public List<TSysModule> GetList(Expression<Func<TSysModule, bool>> predicate)
        {
             using (EdDBContext db=new EdDBContext())
            {
             var models= db.TSysModules.Where<TSysModule>(predicate);
        ...
        public TSysModule GetEntity(Expression<Func<TSysModule, bool>> predicate) 
        {
            using (EdDBContext db=new EdDBContext())
            {
                var model =db.TSysModules.Where<TSysModule>(predicate);
        ...
        public Dictionary<string, object> GetEntityWithExp(string selector, string expFields, string expSelector,
		    ...
		    using (EdDBContext db=new EdDBContext())
            {
                ...
                {
                    model =
                        ((IQueryable<object>) db.TSysModules.Where(predicate, values).Select(selector, values));
                }
                else
                {
                    model = ((IQueryable<object>)db.TSysModules.Where(predicate, values));
                }
        ...
        public  IQueryable<Object> GetFields(string selector, string predicate, params object[] values)
        {
            using (EdDBContext db=new EdDBContext())
            {
                var model = db.TSysModules.Where(predicate,values).Select(selector);
                var sqlText = model.GetProperty("SqlText");
                LogHelper.Debug(sqlText.ToString());
        ...
        /// </summary>
        /// <returns></returns>
       public   bool ExistsEntity(Expression<Func<TSysModule , bool>> predicate)
	   {
            using (EdDBContext db=new EdDBContext())
            {
               bool status= db.TSysModules.Any(predicate);
               return status;
            }
        }
       ...
       public   bool ExistsEntity(string predicate,params object[] values)
	   {
            using (EdDBContext db=new EdDBContext())
            {
               bool status= db.TSysModules.Where(predicate,values).Any();
               return status;
            }
        }
		
	   //查询分页
        public IPagination<TSysModule> GetListByPage(int pageIndex, int pageSize, out int rowCount,
            IDictionary<string, Ed.Entity.EdEnums.OrderEmum> orders,
            Expression<Func<TSysModule, bool>> predicate)
        {
            using (EdDBContext db = new EdDBContext())
            {
                var roles = db.TSysModules.Where(predicate);
	  ...
	  //===========================================================================
	   /// <summary>
        ...
        public DataTable GetListBySql(string sql, dynamic namedParameters)
        {
          using (EdDBContext db=new EdDBContext())
            {
               return db.DbHelper.ExecuteDataTable(sql,namedParameters).ToList<TSysModule>();
            }
   ...
   #region 写操作
        ...
        public bool InsertEntity(TSysModule entity)
        {
            using (EdDBContext db=new EdDBContext())
            {
              int rows=  db.TSysModules.Insert(entity);
				 if (rows > 0)
                {
                    return true;
                }
       ...
       /// <summary>
        ...
        /// </summary>
         /// <param name="predicate">Lamda表达式</param>
        public bool DeleteEntity(Expression<Func<TSysModule , bool>> predicate) 
        {
            using (EdDBContext db=new EdDBContext())
            {
                TSysModule  entity = db.TSysModules.Where(predicate).First();
                int rows=db.TSysModules.Delete(entity);
				 if (rows > 0)
                {
                    return true;
                }
        ...
        public bool DeletesEntity(List<string> predicates)
        {
            using (EdDBContext db=new EdDBContext())
            {
                ...
                try
                {
                     ...
                     {
                        var item = db.TSysModules.Where(predicate).FirstOrDefault();
                        if (null != item)
                         {
                            db.TSysModules.Delete(item);
                         }
                    }
                    tran.Commit();
        ...
        /// </summary>
        /// <param name="entity">实体对象</param>
        public bool UpdateEntity(TSysModule entity)
        {
            using (EdDBContext db=new EdDBContext())
            {
               int rows= db.TSysModules.Update(entity);
			   if (rows > 0)
                {
                    return true;
                }
		 ...
		 using (EdDBContext db=new EdDBContext())
            {
                ...
                try
                {
                    ...
                    {
                     var entity = db.TSysModules.Where(field.Predicate, values).FirstOrDefault();
                        ...
                        db.TSysModules.Update(entity);
                    }


                    tran.Commit();
                    LogHelper.Debug("TSysModule字段批量更新成功。");
                    return true;
                }
                catch (Exception ex)
                {
                    tran.Rollback();
                    LogHelper.Error("TSysModule字段批量更新异常：", ex);
                    return false;
                }
                finally
                {
