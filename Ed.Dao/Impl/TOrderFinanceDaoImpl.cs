...
{
    /// <summary>
    /// 自动生成的实现ITOrderFinanceDao接口的Dao类。 2014-10-28 10:57:52 By 唐有炜
    /// </summary>
    public class TOrderFinanceDaoImpl : ITOrderFinanceDao
    {
        #region 读操作
        ...
        /// <returns>返回所有数据总数</returns>
        public int GetCount(Expression<Func<TOrderFinance, bool>> predicate)
        {
            using (EdDBContext db = new EdDBContext())
            {
                var models = db.TOrderFinances.Where<TOrderFinance>(predicate);
                var sqlText = models.GetProperty("SqlText");
                LogHelper.Debug(sqlText.ToString());
                return models.Count();
            }
        ...
        public List<TOrderFinance> GetList()
        {
            using (EdDBContext db = new EdDBContext())
            {
                var models = db.TOrderFinances;
        ...
        /// <param name="predicate">Lamda表达式</param>
        /// <returns>返回所有数据列表</returns>
        public List<TOrderFinance> GetList(Expression<Func<TOrderFinance, bool>> predicate)
        {
            using (EdDBContext db = new EdDBContext())
            {
                var models = db.TOrderFinances.Where<TOrderFinance>(predicate);
        ...
        public TOrderFinance GetEntity(Expression<Func<TOrderFinance, bool>> predicate)
        {
            using (EdDBContext db = new EdDBContext())
            {
                var model = db.TOrderFinances.Where<TOrderFinance>(predicate);
            ...
            params object[] values)
        {
            using (EdDBContext db = new EdDBContext())
            {
                ...
                {
                    model =
                        ((IQueryable<object>) db.TOrderFinances.Where(predicate, values).Select(selector, values));
                }
                else
                {
                    model = ((IQueryable<object>) db.TOrderFinances.Where(predicate, values));
                }
        ...
        public IQueryable<Object> GetFields(string selector, string predicate, params object[] values)
        {
            using (EdDBContext db = new EdDBContext())
            {
                var model = db.TOrderFinances.Where(predicate, values).Select(selector);
                var sqlText = model.GetProperty("SqlText");
                LogHelper.Debug(sqlText.ToString());
        ...
        /// </summary>
        /// <returns></returns>
        public bool ExistsEntity(Expression<Func<TOrderFinance, bool>> predicate)
        {
            using (EdDBContext db = new EdDBContext())
            {
                bool status = db.TOrderFinances.Any(predicate);
                return status;
            }
        }
        ...
        public bool ExistsEntity(string predicate, params object[] values)
        {
            using (EdDBContext db = new EdDBContext())
            {
                bool status = db.TOrderFinances.Where(predicate, values).Any();
                return status;
            }
        }

        //查询分页
        public IPagination<TOrderFinance> GetListByPage(int pageIndex, int pageSize, out int rowCount,
            IDictionary<string, Ed.Entity.EdEnums.OrderEmum> orders,
            Expression<Func<TOrderFinance, bool>> predicate)
        {
            using (EdDBContext db = new EdDBContext())
            {
                var roles = db.TOrderFinances.Where(predicate);
        ...
        public DataTable GetListBySql(string sql, dynamic namedParameters)
        {
            using (EdDBContext db = new EdDBContext())
            {
                return db.DbHelper.ExecuteDataTable(sql, namedParameters).ToList<TOrderFinance>();
            }
        }
        ...
        public bool InsertEntity(TOrderFinance entity)
        {
            using (EdDBContext db = new EdDBContext())
            {
                int rows = db.TOrderFinances.Insert(entity);
                if (rows > 0)
                {
                    return true;
                }
        ...
        /// </summary>
        /// <param name="predicate">Lamda表达式</param>
        public bool DeleteEntity(Expression<Func<TOrderFinance, bool>> predicate)
        {
            using (EdDBContext db = new EdDBContext())
            {
                TOrderFinance entity = db.TOrderFinances.Where(predicate).First();
                int rows = db.TOrderFinances.Delete(entity);
                if (rows > 0)
                {
                    return true;
                }
        ...
        public bool DeletesEntity(List<string> predicates)
        {
            using (EdDBContext db = new EdDBContext())
            {
                ...
                try
                {
                    ...
                    {
                        var item = db.TOrderFinances.Where(predicate).FirstOrDefault();
                        if (null != item)
                        {
                            db.TOrderFinances.Delete(item);
                        }
                    }
                    tran.Commit();
        ...
        /// </summary>
        /// <param name="entity">实体对象</param>
        public bool UpdateEntity(TOrderFinance entity)
        {
            using (EdDBContext db = new EdDBContext())
            {
                int rows = db.TOrderFinances.Update(entity);
                if (rows > 0)
                {
                    return true;
                }
        ...
        public bool UpdateEntityStatus(List<Field> fields, params object[] values)
        {
            using (EdDBContext db = new EdDBContext())
            {
                ...
                try
                {
                    ...
                    {
                        var entity = db.TOrderFinances.Where(field.Predicate, values).FirstOrDefault();
                        ...
                        db.TOrderFinances.Update(entity);
                    }


                    tran.Commit();
                    LogHelper.Debug("TOrderFinance字段批量更新成功。");
                    return true;
                }
                catch (Exception ex)
                {
                    tran.Rollback();
                    LogHelper.Error("TOrderFinance字段批量更新异常：", ex);
                    return false;
                }
                finally
                {
        ...
        /// <returns><c>true</c> if XXXX, <c>false</c> otherwise.</returns>
        public bool InsertEntityTran(TOrderFinance orderFinance)
        {
            using (EdDBContext db = new EdDBContext())
            {
                if (db.Connection.State != ConnectionState.Open)
                ...
                try
                {
                    ...
                    //更新已付款
                    var orderCode = orderFinance.OrderCode;
                    var order = db.TOrders.FirstOrDefault(o => o.OrderCode == orderCode);
                    order.OrderPrepaid += orderFinance.FinPayment;
                    db.TOrders.Update(order);
                
                    tran.Commit();
                    LogHelper.Debug("订单收款完成。金额：" + order.OrderPrepaid);
                    return true;
                }
                catch (Exception ex)
                {
                    tran.Rollback();
                    LogHelper.Error("订单收款失败,异常：", ex);
                    return false;
                }
                finally
                {
