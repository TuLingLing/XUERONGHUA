            ...
            using (EdDBContext db = new EdDBContext())
            {
                ...
                foreach (var page in pages)
                {
                    ...
                    foreach (var propertyInfo in propertyInfos)
                    {
                        ...
                        {
                            case "DepId":
                                int DepId = int.Parse(value.ToString());
                                object objDepId =
                                    db.TSysDepartments.Where(d => d.Id == DepId)
                                        .Select(d => d.DepName)
                                        .FirstOrDefault();
                                var DepName = "";
                                if (null != objDepId)
                                {
                                    DepName = objDepId.ToString();
                                    result.Add("DepName", DepName);
                                }
                                break;
                            case "RoleId": //维护人
                                int RoleId = int.Parse(value.ToString());
                                object objRoleId =
                                    db.TSysRoles.Where(r => r.Id == RoleId)
                                        .Select(r => r.RoleName)
                                        .FirstOrDefault();
                                var RoleName = "";
                                if (null != objRoleId)
                                {
                                    RoleName = objRoleId.ToString();
