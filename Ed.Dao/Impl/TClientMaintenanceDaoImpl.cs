...
namespace Ed.Dao.Impl
{
    ...
    /// <summary>
    /// 自动生成的实现ITClientMaintenanceDao接口的Dao类。 2014-10-15 10:05:36 By 唐有炜
    /// </summary>
    public class TClientMaintenanceDaoImpl : ITClientMaintenanceDao
    {
        ...
        public int GetCount()
        {
            ...
            {
                var models = db.TClientMaintenances;
        ...
        /// <returns>返回所有数据总数</returns>
        public int GetCount(Expression<Func<TClientMaintenance, bool>> predicate)
        {
            using (EdDBContext db = new EdDBContext())
            {
                var models = db.TClientMaintenances.Where<TClientMaintenance>(predicate);
                var sqlText = models.GetProperty("SqlText");
                LogHelper.Debug(sqlText.ToString());
                return models.Count();
            }
        ...
        public List<TClientMaintenance> GetList()
        {
            using (EdDBContext db = new EdDBContext())
            {
                var models = db.TClientMaintenances;
        ...
        /// <param name="predicate">Lamda表达式</param>
        /// <returns>返回所有数据列表</returns>
        public List<TClientMaintenance> GetList(Expression<Func<TClientMaintenance, bool>> predicate)
        {
            using (EdDBContext db = new EdDBContext())
            {
                var models = db.TClientMaintenances.Where<TClientMaintenance>(predicate);
        ...
        public TClientMaintenance GetEntity(Expression<Func<TClientMaintenance, bool>> predicate)
        {
            using (EdDBContext db = new EdDBContext())
            {
                var model = db.TClientMaintenances.Where<TClientMaintenance>(predicate);
            ...
            params object[] values)
        {
            using (EdDBContext db = new EdDBContext())
            {
                ...
                {
                    model =
                        ((IQueryable<object>)db.TClientMaintenances.Where(predicate, values).Select(selector, values));
                }
                else
                {
                    model = ((IQueryable<object>)db.TClientMaintenances.Where(predicate, values));
                }
        ...
        public IQueryable<Object> GetFields(string selector, string predicate, params object[] values)
        {
            using (EdDBContext db = new EdDBContext())
            {
                var model = db.TClientMaintenances.Where(predicate, values).Select(selector);
                var sqlText = model.GetProperty("SqlText");
                LogHelper.Debug(sqlText.ToString());
        ...
        /// </summary>
        /// <returns></returns>
        public bool ExistsEntity(Expression<Func<TClientMaintenance, bool>> predicate)
        {
            using (EdDBContext db = new EdDBContext())
            {
                bool status = db.TClientMaintenances.Any(predicate);
                return status;
            }
        }
        ...
        public bool ExistsEntity(string predicate, params object[] values)
        {
            using (EdDBContext db = new EdDBContext())
            {
                bool status = db.TClientMaintenances.Where(predicate,values).Any();
                return status;
            }
        }
        ...
        //查询分页
        public IPagination<TClientMaintenance> GetListByPage(int pageIndex, int pageSize, out int rowCount,
            IDictionary<string, Ed.Entity.EdEnums.OrderEmum> orders,
            Expression<Func<TClientMaintenance, bool>> predicate)
        {
            using (EdDBContext db = new EdDBContext())
            {
                var roles = db.TClientMaintenances.Where(predicate);
                 ...
                 out int recordCount, params object[] values)
        {
            ...
            using (EdDBContext db = new EdDBContext())
            {
                //获取查询结果
                //加上扩展字段值
                var temps = db.TClientMaintenances;
                var prevCount = (pageIndex - 1) * pageSize;
                ...
                foreach (var page in pages)
                {
                    ...
                    foreach (var propertyInfo in propertyInfos)
                    {
                        ...
                        {
                            case "CMainCreater":
                                int CMainCreater = int.Parse(value.ToString());
                                object objCMainCreater =
                                    db.TSysUsers.Where(u => u.Id == CMainCreater).Select(u => u.UserTname).FirstOrDefault();
                                var CMainCreaterName = "";
                                if (null != objCMainCreater)
                                {
                                    CMainCreaterName = objCMainCreater.ToString();
                                    result.Add("CMainCreaterName", CMainCreaterName);
                                }
                                break;
                            default://添加默认字段
                                result.Add(name, value);
        ...
        public DataTable GetListBySql(string sql, dynamic namedParameters)
        {
            using (EdDBContext db = new EdDBContext())
            {
                return db.DbHelper.ExecuteDataTable(sql, namedParameters).ToList<TClientMaintenance>();
            }
        ...
        public bool InsertEntity(TClientMaintenance entity)
        {
            using (EdDBContext db = new EdDBContext())
            {
                int rows = db.TClientMaintenances.Insert(entity);
                if (rows > 0)
                {
                    return true;
                }
        ...
        /// </summary>
        /// <param name="predicate">Lamda表达式</param>
        public bool DeleteEntity(Expression<Func<TClientMaintenance, bool>> predicate)
        {
            using (EdDBContext db = new EdDBContext())
            {
                TClientMaintenance entity = db.TClientMaintenances.Where(predicate).First();
                int rows = db.TClientMaintenances.Delete(entity);
                if (rows > 0)
                {
                    return true;
                }
        ...
        public bool DeletesEntity(List<string> predicates)
        {
            using (EdDBContext db = new EdDBContext())
            {
                ...
                try
                {
                    ...
                    {
                        var item = db.TClientMaintenances.Where(predicate).FirstOrDefault();
                        if (null != item)
                        {
                            db.TClientMaintenances.Delete(item);
                        }
                    }
                    tran.Commit();
        ...
        /// </summary>
        /// <param name="entity">实体对象</param>
        public bool UpdateEntity(TClientMaintenance entity)
        {
            using (EdDBContext db = new EdDBContext())
            {
                int rows = db.TClientMaintenances.Update(entity);
                if (rows > 0)
                {
                    return true;
                }
        ...
        /// <summary>
        /// 使用LINQ批量更改TClientMaintenance状态 2014-09-05 14:58:50 By 唐有炜
        /// </summary>
             ...
             string predicate,params  object[] values)
        {
            ...
            using (EdDBContext db = new EdDBContext())
            {
                ...
                try
                {
                    ...
                    {
                        var entity = db.TClientMaintenances.Where(predicate,values).FirstOrDefault();
                        ...
                        db.TClientMaintenances.Update(entity);
                    }


                    tran.Commit();
                    LogHelper.Debug("TClientMaintenance字段批量更新成功。");
                    return true;
                }
                catch (Exception ex)
                {
                    tran.Rollback();
                    LogHelper.Error("TClientMaintenance字段批量更新异常：", ex);
                    return false;
                }
                finally
                {
