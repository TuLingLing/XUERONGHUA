...
{
    /// <summary>
    /// 自动生成的实现ITSysPowerDao接口的Dao类。 2014-10-20 04:45:28 By 唐有炜
    /// </summary>
    public class TSysPowerDaoImpl : ITSysPowerDao
    {
        ...
        /// <summary>
        /// 使用LINQ批量更改TSysPower状态 2014-09-05 14:58:50 By 唐有炜
        /// </summary>
            ...
            string predicate, params  object[] values)
        {
            using (EdDBContext db = new EdDBContext())
            {
                ...
                try
                {
                    ...
                    {
                        var entity = db.TSysPowers.Where(predicate,values).FirstOrDefault();
            ...
            params object[] values)
        {
            using (EdDBContext db = new EdDBContext())
            {
                var modules = db.TSysModules;
                var powers = db.TSysPowers;
                var temps = powers
                    .Join(modules, p => p.Module, m => m.Id, (power, moudle) => new
                    {
                        power.Id, //权限id
                        power.PowerAction,
                        power.RoleId, //角色id
                ...
                {
                    Dictionary<string, object> result = new Dictionary<string, object>();
                    PropertyInfo[] propertyInfos = list.GetType().GetProperties();
                    foreach (var propertyInfo in propertyInfos)
                    {
                        var name = propertyInfo.Name; //输入的selector中的字段名
                        var value = propertyInfo.GetValue(list, null); //输入的selector中的字段值
                        //可以添加其他的属性
                        result.Add(name, value);
                    }
                    results.Add(result);
...
//                if (!string.IsNullOrEmpty(selector))
//                {
//                    results = results.Select(selector, values);
//                }

//                if (!string.IsNullOrEmpty(predicate))
//                {
//                    results = results.AsQueryable().Where(predicate, values).ToList();
//                }
