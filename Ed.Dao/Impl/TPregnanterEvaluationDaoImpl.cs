...
namespace  Ed.Dao.Impl
{
    ...
    /// <summary>
    /// 自动生成的实现ITPregnanterEvaluationDao接口的Dao类。 2014-10-15 10:05:36 By 唐有炜
    /// </summary>
 public class TPregnanterEvaluationDaoImpl:ITPregnanterEvaluationDao
    {
	  ...
	  #region 读操作
	   ...
	   /// <summary>
        ...
        public int GetCount() 
        {
          using (EdDBContext db=new EdDBContext())
            {
             var models= db.TPregnanterEvaluations;
        ...
        /// <returns>返回所有数据总数</returns>
       public int GetCount(Expression<Func<TPregnanterEvaluation, bool>> predicate)
        {
             using (EdDBContext db=new EdDBContext())
            {
             var models= db.TPregnanterEvaluations.Where<TPregnanterEvaluation>(predicate);
			 var sqlText = models.GetProperty("SqlText");
             LogHelper.Debug(sqlText.ToString());
			 return models.Count();
            }
        ...
        public List<TPregnanterEvaluation> GetList() 
        {
          using (EdDBContext db=new EdDBContext())
            {
             var models= db.TPregnanterEvaluations;
        ...
        /// <param name="predicate">Lamda表达式</param>
        /// <returns>返回所有数据列表</returns>
       public List<TPregnanterEvaluation> GetList(Expression<Func<TPregnanterEvaluation, bool>> predicate)
        {
             using (EdDBContext db=new EdDBContext())
            {
             var models= db.TPregnanterEvaluations.Where<TPregnanterEvaluation>(predicate);
        ...
        public TPregnanterEvaluation GetEntity(Expression<Func<TPregnanterEvaluation, bool>> predicate) 
        {
            using (EdDBContext db=new EdDBContext())
            {
                var model =db.TPregnanterEvaluations.Where<TPregnanterEvaluation>(predicate);
        ...
        public Dictionary<string, object> GetEntityWithExp(string selector, string expFields, string expSelector,
		    ...
		    using (EdDBContext db=new EdDBContext())
            {
                ...
                {
                    model =
                        ((IQueryable<object>) db.TPregnanterEvaluations.Where(predicate, values).Select(selector, values));
                }
                else
                {
                    model = ((IQueryable<object>)db.TPregnanterEvaluations.Where(predicate, values));
                }
        ...
        public IQueryable<Object> GetFields(string selector, string predicate, params object[] values)
        {
            using (EdDBContext db = new EdDBContext())
            {
                var model = db.TPregnanterEvaluations.Where(predicate, values).Select(selector);
                var sqlText = model.GetProperty("SqlText");
                LogHelper.Debug(sqlText.ToString());
        ...
        /// </summary>
        /// <returns></returns>
       public   bool ExistsEntity(Expression<Func<TPregnanterEvaluation , bool>> predicate)
	   {
            using (EdDBContext db=new EdDBContext())
            {
               bool status= db.TPregnanterEvaluations.Any(predicate);
               return status;
            }
        }
       ...
       public bool ExistsEntity(string predicate, params object[] values)
       {
           using (EdDBContext db = new EdDBContext())
           {
               bool status = db.TPregnanterEvaluations.Where(predicate,values).Any();
               return status;
           }
       }
	   ...
	   //查询分页
        public IPagination<TPregnanterEvaluation> GetListByPage(int pageIndex, int pageSize, out int rowCount,
            IDictionary<string, Ed.Entity.EdEnums.OrderEmum> orders,
            Expression<Func<TPregnanterEvaluation, bool>> predicate)
        {
            using (EdDBContext db = new EdDBContext())
            {
                var roles = db.TPregnanterEvaluations.Where(predicate);
		  ...
		  using (EdDBContext db=new EdDBContext())
            {
                //获取查询结果
                //加上扩展字段值
                var temps = db.TPregnanterEvaluations;
                var prevCount = (pageIndex - 1)*pageSize;
	  ...
	  //===========================================================================
	   /// <summary>
        ...
        public DataTable GetListBySql(string sql, dynamic namedParameters)
        {
          using (EdDBContext db=new EdDBContext())
            {
               return db.DbHelper.ExecuteDataTable(sql,namedParameters).ToList<TPregnanterEvaluation>();
            }
   ...
   #region 写操作
        ...
        public bool InsertEntity(TPregnanterEvaluation entity)
        {
            using (EdDBContext db=new EdDBContext())
            {
              int rows=  db.TPregnanterEvaluations.Insert(entity);
				 if (rows > 0)
                {
                    return true;
                }
       ...
       /// <summary>
        ...
        /// </summary>
         /// <param name="predicate">Lamda表达式</param>
        public bool DeleteEntity(Expression<Func<TPregnanterEvaluation , bool>> predicate) 
        {
            using (EdDBContext db=new EdDBContext())
            {
                TPregnanterEvaluation  entity = db.TPregnanterEvaluations.Where(predicate).First();
                int rows=db.TPregnanterEvaluations.Delete(entity);
				 if (rows > 0)
                {
                    return true;
                }
        ...
        public bool DeletesEntity(List<string> predicates)
        {
            using (EdDBContext db=new EdDBContext())
            {
                ...
                try
                {
                     ...
                     {
                        var item = db.TPregnanterEvaluations.Where(predicate).FirstOrDefault();
                        if (null != item)
                         {
                            db.TPregnanterEvaluations.Delete(item);
                         }
                    }
                    tran.Commit();
        ...
        /// </summary>
        /// <param name="entity">实体对象</param>
        public bool UpdateEntity(TPregnanterEvaluation entity)
        {
            using (EdDBContext db=new EdDBContext())
            {
               int rows= db.TPregnanterEvaluations.Update(entity);
			   if (rows > 0)
                {
                    return true;
                }
        ...
        /// <summary>
        /// 使用LINQ批量更改TPregnanterEvaluation状态 2014-09-05 14:58:50 By 唐有炜
        /// </summary>
		 ...
		 using (EdDBContext db=new EdDBContext())
            {
                ...
                try
                {
                    ...
                    {
                     var entity = db.TPregnanterEvaluations.Where(predicate,values).FirstOrDefault();
                        ...
                        db.TPregnanterEvaluations.Update(entity);
                    }


                    tran.Commit();
                    LogHelper.Debug("TPregnanterEvaluation字段批量更新成功。");
                    return true;
                }
                catch (Exception ex)
                {
                    tran.Rollback();
                    LogHelper.Error("TPregnanterEvaluation字段批量更新异常：", ex);
                    return false;
                }
                finally
                {
