...
{
    /// <summary>
    /// 自动生成的实现ITPregnanterInfoDao接口的Dao类。 2014-10-15 10:05:36 By 唐有炜
    /// </summary>
    public class TPregnanterInfoDaoImpl : ITPregnanterInfoDao
    {
        #region 读操作
        ...
        public int GetCount()
        {
            ...
            {
                var models = db.TPregnanterInfos;
        ...
        public int GetMaxId()
        {
            using (EdDBContext db = new EdDBContext())
            {
                var models = db.TPregnanterInfos.Max(temp => temp.Id);
                //var sqlText = models.GetProperty("SqlText");
                //LogHelper.Debug(sqlText.ToString());
                return models + 1;
            }
        ...
        /// <returns>返回所有数据总数</returns>
        public int GetCount(Expression<Func<TPregnanterInfo, bool>> predicate)
        {
            using (EdDBContext db = new EdDBContext())
            {
                var models = db.TPregnanterInfos.Where<TPregnanterInfo>(predicate);
                var sqlText = models.GetProperty("SqlText");
                LogHelper.Debug(sqlText.ToString());
                return models.Count();
            }
        ...
        public List<TPregnanterInfo> GetList()
        {
            using (EdDBContext db = new EdDBContext())
            {
                var models = db.TPregnanterInfos;
        ...
        /// <param name="predicate">Lamda表达式</param>
        /// <returns>返回所有数据列表</returns>
        public List<TPregnanterInfo> GetList(Expression<Func<TPregnanterInfo, bool>> predicate)
        {
            using (EdDBContext db = new EdDBContext())
            {
                var models = db.TPregnanterInfos.Where<TPregnanterInfo>(predicate);
        ...
        public TPregnanterInfo GetEntity(Expression<Func<TPregnanterInfo, bool>> predicate)
        {
            using (EdDBContext db = new EdDBContext())
            {
                var model = db.TPregnanterInfos.Where<TPregnanterInfo>(predicate);
            ...
            params object[] values)
        {
            using (EdDBContext db = new EdDBContext())
            {
                ...
                {
                    model =
                        ((IQueryable<object>) db.TPregnanterInfos.Where(predicate, values).Select(selector, values));
                }
                else
                {
                    model = ((IQueryable<object>) db.TPregnanterInfos.Where(predicate, values));
                }
        ...
        public IQueryable<Object> GetFields(string selector, string predicate, params object[] values)
        {
            using (EdDBContext db = new EdDBContext())
            {
                var model = db.TPregnanterInfos.Where(predicate, values).Select(selector);
                var sqlText = model.GetProperty("SqlText");
                LogHelper.Debug(sqlText.ToString());
        ...
        /// </summary>
        /// <returns></returns>
        public bool ExistsEntity(Expression<Func<TPregnanterInfo, bool>> predicate)
        {
            using (EdDBContext db = new EdDBContext())
            {
                bool status = db.TPregnanterInfos.Any(predicate);
                return status;
            }
        }
        ...
        public bool ExistsEntity(string predicate, params object[] values)
        {
            using (EdDBContext db = new EdDBContext())
            {
                bool status = db.TPregnanterInfos.Where(predicate, values).Any();
                return status;
            }
        }
        ...
        //查询分页
        public IPagination<TPregnanterInfo> GetListByPage(int pageIndex, int pageSize, out int rowCount,
            IDictionary<string, Ed.Entity.EdEnums.OrderEmum> orders,
            Expression<Func<TPregnanterInfo, bool>> predicate)
        {
            using (EdDBContext db = new EdDBContext())
            {
                var roles = db.TPregnanterInfos.Where(predicate);
        ...
        public DataTable GetListBySql(string sql, dynamic namedParameters)
        {
            using (EdDBContext db = new EdDBContext())
            {
                return db.DbHelper.ExecuteDataTable(sql, namedParameters).ToList<TPregnanterInfo>();
            }
        }
        ...
        public bool InsertEntity(TPregnanterInfo entity)
        {
            using (EdDBContext db = new EdDBContext())
            {
                int rows = db.TPregnanterInfos.Insert(entity);
                if (rows > 0)
                {
                    return true;
                }
        ...
        /// </summary>
        /// <param name="predicate">Lamda表达式</param>
        public bool DeleteEntity(Expression<Func<TPregnanterInfo, bool>> predicate)
        {
            using (EdDBContext db = new EdDBContext())
            {
                TPregnanterInfo entity = db.TPregnanterInfos.Where(predicate).First();
                int rows = db.TPregnanterInfos.Delete(entity);
                if (rows > 0)
                {
                    return true;
                }
        ...
        public bool DeletesEntity(List<string> predicates)
        {
            using (EdDBContext db = new EdDBContext())
            {
                ...
                try
                {
                    ...
                    {
                        var item = db.TPregnanterInfos.Where(predicate).FirstOrDefault();
                        if (null != item)
                        {
                            db.TPregnanterInfos.Delete(item);
                        }
                    }
                    tran.Commit();
        ...
        /// </summary>
        /// <param name="entity">实体对象</param>
        public bool UpdateEntity(TPregnanterInfo entity)
        {
            using (EdDBContext db = new EdDBContext())
            {
                int rows = db.TPregnanterInfos.Update(entity);
                if (rows > 0)
                {
                    return true;
                }
        ...
        /// <summary>
        /// 使用LINQ批量更改TPregnanterInfo状态 2014-09-05 14:58:50 By 唐有炜
        /// </summary>
            ...
            params object[] values)
        {
            using (EdDBContext db = new EdDBContext())
            {
                ...
                try
                {
                    ...
                    {
                        var entity = db.TPregnanterInfos.Where(field.Predicate, values).FirstOrDefault();
                        ...
                        db.TPregnanterInfos.Update(entity);
                    }


                    tran.Commit();
                    LogHelper.Debug("TPregnanterInfo字段批量更新成功。");
                    return true;
                }
                catch (Exception ex)
                {
                    tran.Rollback();
                    LogHelper.Error("TPregnanterInfo字段批量更新异常：", ex);
                    return false;
                }
                finally
                {
