...
namespace  Ed.Dao.Impl
{
    ...
    /// <summary>
    /// 自动生成的实现ITPregnanterMaintenanceDao接口的Dao类。 2014-10-15 10:05:36 By 唐有炜
    /// </summary>
 public class TPregnanterMaintenanceDaoImpl:ITPregnanterMaintenanceDao
    {
	  ...
	  #region 读操作
	   ...
	   /// <summary>
        ...
        public int GetCount() 
        {
          using (EdDBContext db=new EdDBContext())
            {
             var models= db.TPregnanterMaintenances;
        ...
        /// <returns>返回所有数据总数</returns>
       public int GetCount(Expression<Func<TPregnanterMaintenance, bool>> predicate)
        {
             using (EdDBContext db=new EdDBContext())
            {
             var models= db.TPregnanterMaintenances.Where<TPregnanterMaintenance>(predicate);
			 var sqlText = models.GetProperty("SqlText");
             LogHelper.Debug(sqlText.ToString());
			 return models.Count();
            }
        ...
        public List<TPregnanterMaintenance> GetList() 
        {
          using (EdDBContext db=new EdDBContext())
            {
             var models= db.TPregnanterMaintenances;
        ...
        /// <param name="predicate">Lamda表达式</param>
        /// <returns>返回所有数据列表</returns>
       public List<TPregnanterMaintenance> GetList(Expression<Func<TPregnanterMaintenance, bool>> predicate)
        {
             using (EdDBContext db=new EdDBContext())
            {
             var models= db.TPregnanterMaintenances.Where<TPregnanterMaintenance>(predicate);
        ...
        public TPregnanterMaintenance GetEntity(Expression<Func<TPregnanterMaintenance, bool>> predicate) 
        {
            using (EdDBContext db=new EdDBContext())
            {
                var model =db.TPregnanterMaintenances.Where<TPregnanterMaintenance>(predicate);
        ...
        public Dictionary<string, object> GetEntityWithExp(string selector, string expFields, string expSelector,
		    ...
		    using (EdDBContext db=new EdDBContext())
            {
                ...
                {
                    model =
                        ((IQueryable<object>) db.TPregnanterMaintenances.Where(predicate, values).Select(selector, values));
                }
                else
                {
                    model = ((IQueryable<object>)db.TPregnanterMaintenances.Where(predicate, values));
                }
        ...
        public IQueryable<Object> GetFields(string selector, string predicate, params object[] values)
        {
            using (EdDBContext db = new EdDBContext())
            {
                var model = db.TPregnanterMaintenances.Where(predicate, values).Select(selector);
                var sqlText = model.GetProperty("SqlText");
                LogHelper.Debug(sqlText.ToString());
        ...
        /// </summary>
        /// <returns></returns>
       public   bool ExistsEntity(Expression<Func<TPregnanterMaintenance , bool>> predicate)
	   {
            using (EdDBContext db=new EdDBContext())
            {
               bool status= db.TPregnanterMaintenances.Any(predicate);
               return status;
            }
        }
       ...
       public bool ExistsEntity(string predicate, params object[] values)
       {
           using (EdDBContext db = new EdDBContext())
           {
               bool status = db.TPregnanterMaintenances.Where(predicate,values).Any();
               return status;
           }
       }
	   ...
	   //查询分页
        public IPagination<TPregnanterMaintenance> GetListByPage(int pageIndex, int pageSize, out int rowCount,
            IDictionary<string, Ed.Entity.EdEnums.OrderEmum> orders,
            Expression<Func<TPregnanterMaintenance, bool>> predicate)
        {
            using (EdDBContext db = new EdDBContext())
            {
                var roles = db.TPregnanterMaintenances.Where(predicate);
		  ...
		  using (EdDBContext db=new EdDBContext())
            {
                //获取查询结果
                //加上扩展字段值
                var temps = db.TPregnanterMaintenances;
                var prevCount = (pageIndex - 1)*pageSize;
                ...
                foreach (var page in pages)
                {
                    ...
                    foreach (var propertyInfo in propertyInfos)
                    {
                        ...
                        {
                            case "PMainCreater":
                                int PMainCreater = int.Parse(value.ToString());
                                object objPMainCreater =
                                    db.TSysUsers.Where(u => u.Id == PMainCreater)
                                        .Select(u => u.UserTname)
                                        .FirstOrDefault();
                                var PMainCreaterName = "";
                                if (null != objPMainCreater)
                                {
                                    PMainCreaterName = objPMainCreater.ToString();
                                    result.Add("PMainCreaterName", PMainCreaterName);
                                }
                                break;
                            default: //添加默认字段
                                result.Add(name, value);
	  ...
	  //===========================================================================
	   /// <summary>
        ...
        public DataTable GetListBySql(string sql, dynamic namedParameters)
        {
          using (EdDBContext db=new EdDBContext())
            {
               return db.DbHelper.ExecuteDataTable(sql,namedParameters).ToList<TPregnanterMaintenance>();
            }
   ...
   #region 写操作
        ...
        public bool InsertEntity(TPregnanterMaintenance entity)
        {
            using (EdDBContext db=new EdDBContext())
            {
              int rows=  db.TPregnanterMaintenances.Insert(entity);
				 if (rows > 0)
                {
                    return true;
                }
       ...
       /// <summary>
        ...
        /// </summary>
         /// <param name="predicate">Lamda表达式</param>
        public bool DeleteEntity(Expression<Func<TPregnanterMaintenance , bool>> predicate) 
        {
            using (EdDBContext db=new EdDBContext())
            {
                TPregnanterMaintenance  entity = db.TPregnanterMaintenances.Where(predicate).First();
                int rows=db.TPregnanterMaintenances.Delete(entity);
				 if (rows > 0)
                {
                    return true;
                }
        ...
        public bool DeletesEntity(List<string> predicates)
        {
            using (EdDBContext db=new EdDBContext())
            {
                ...
                try
                {
                     ...
                     {
                        var item = db.TPregnanterMaintenances.Where(predicate).FirstOrDefault();
                        if (null != item)
                         {
                            db.TPregnanterMaintenances.Delete(item);
                         }
                    }
                    tran.Commit();
        ...
        /// </summary>
        /// <param name="entity">实体对象</param>
        public bool UpdateEntity(TPregnanterMaintenance entity)
        {
            using (EdDBContext db=new EdDBContext())
            {
               int rows= db.TPregnanterMaintenances.Update(entity);
			   if (rows > 0)
                {
                    return true;
                }
        ...
        /// <summary>
        /// 使用LINQ批量更改TPregnanterMaintenance状态 2014-09-05 14:58:50 By 唐有炜
        /// </summary>
		 ...
		 using (EdDBContext db=new EdDBContext())
            {
                ...
                try
                {
                    ...
                    {
                     var entity = db.TPregnanterMaintenances.Where(predicate,values).FirstOrDefault();
                        ...
                        db.TPregnanterMaintenances.Update(entity);
                    }


                    tran.Commit();
                    LogHelper.Debug("TPregnanterMaintenance字段批量更新成功。");
                    return true;
                }
                catch (Exception ex)
                {
                    tran.Rollback();
                    LogHelper.Error("TPregnanterMaintenance字段批量更新异常：", ex);
                    return false;
                }
                finally
                {
