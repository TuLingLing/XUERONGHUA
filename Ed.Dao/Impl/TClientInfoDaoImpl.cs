...
// ***********************************************************************
// <copyright file="TClientInfoDaoImpl.cs" company="郑州优创软件科技有限公司">
...
{
    /// <summary>
    /// 自动生成的实现ITClientInfoDao接口的Dao类。 2014-10-15 10:05:36 By 唐有炜
    /// </summary>
    public class TClientInfoDaoImpl : ITClientInfoDao
    {
        #region 读操作
        ...
        /// <returns>返回所有数据总数</returns>
        public int GetCount(Expression<Func<TClientInfo, bool>> predicate)
        {
            using (EdDBContext db = new EdDBContext())
            {
                var models = db.TClientInfos.Where<TClientInfo>(predicate);
                var sqlText = models.GetProperty("SqlText");
                LogHelper.Debug(sqlText.ToString());
                return models.Count();
            }
        ...
        public int GetMaxId()
        {
            using (EdDBContext db = new EdDBContext())
            {
                var models = db.TClientInfos.Max(temp => temp.Id);
                //var sqlText = models.GetProperty("SqlText");
                //LogHelper.Debug(sqlText.ToString());
                return models + 1;
            }
        ...
        public List<TClientInfo> GetList()
        {
            using (EdDBContext db = new EdDBContext())
            {
                var models = db.TClientInfos;
        ...
        /// <param name="predicate">Lamda表达式</param>
        /// <returns>返回所有数据列表</returns>
        public List<TClientInfo> GetList(Expression<Func<TClientInfo, bool>> predicate)
        {
            using (EdDBContext db = new EdDBContext())
            {
                var models = db.TClientInfos.Where<TClientInfo>(predicate);
        ...
        public TClientInfo GetEntity(Expression<Func<TClientInfo, bool>> predicate)
        {
            using (EdDBContext db = new EdDBContext())
            {
                var model = db.TClientInfos.Where<TClientInfo>(predicate);
            ...
            params object[] values)
        {
            using (EdDBContext db = new EdDBContext())
            {
                ...
                {
                    model =
                        ((IQueryable<object>) db.TClientInfos.Where(predicate, values).Select(selector, values));
                }
                else
                {
                    model = ((IQueryable<object>) db.TClientInfos.Where(predicate, values));
                }
        ...
        public IQueryable<Object> GetFields(string selector, string predicate, params object[] values)
        {
            using (EdDBContext db = new EdDBContext())
            {
                var model = db.TClientInfos.Where(predicate, values).Select(selector);
                var sqlText = model.GetProperty("SqlText");
                LogHelper.Debug(sqlText.ToString());
        ...
        public bool ExistsEntity(Expression<Func<TClientInfo, bool>> predicate)
        {
            using (EdDBContext db = new EdDBContext())
            {
                bool status = db.TClientInfos.Any(predicate);
                return status;
            }
        }
        ...
        public bool ExistsEntity(string predicate, params object[] values)
        {
            using (EdDBContext db = new EdDBContext())
            {
                bool status = db.TClientInfos.Where(predicate, values).Any();
                return status;
            }
        }
        ...
        /// <param name="predicate">查询条件</param>
        /// <returns>IPagination&lt;TClientInfo&gt;.</returns>
        public IPagination<TClientInfo> GetListByPage(int pageIndex, int pageSize, out int rowCount,
            IDictionary<string, Ed.Entity.EdEnums.OrderEmum> orders,
            Expression<Func<TClientInfo, bool>> predicate)
        {
            using (EdDBContext db = new EdDBContext())
            {
                var roles = db.TClientInfos.Where(predicate);
        ...
        public List<Dictionary<string, object>> GetListByPage(int pageIndex, int pageSize, string selector,
            ...
            using (EdDBContext db = new EdDBContext())
            {
                ...
                foreach (var page in pages)
                {
                    ...
                    foreach (var propertyInfo in propertyInfos)
                    {
                        ...
                        {
                            case "ClientCreater":
                                int ClientCreater = int.Parse(value.ToString());
                                object objClientCreater =
                                    db.TSysUsers.Where(u => u.Id == ClientCreater)
                                        .Select(u => u.UserTname)
                                        .FirstOrDefault();
                                var ClientCreaterName = "";
                                if (null != objClientCreater)
                                {
                                    ClientCreaterName = objClientCreater.ToString();
                                    result.Add("ClientCreaterName", ClientCreaterName);
                                }
                                break;
                            case "ClientWeihu": //维护人
                                int ClientWeihu = int.Parse(value.ToString());
                                object objClientWeihu =
                                    db.TSysUsers.Where(u => u.Id == ClientWeihu)
                                        .Select(u => u.UserTname)
                                        .FirstOrDefault();
                                var ClientWeihuName = "";
                                if (null != objClientWeihu)
                                {
                                    ClientWeihuName = objClientWeihu.ToString();
                                    result.Add("ClientWeihuName", ClientWeihuName);
                                }
                                continue;
                            default: //添加默认字段
                                result.Add(name, value);
        ...
        public DataTable GetListBySql(string sql, dynamic namedParameters)
        {
            using (EdDBContext db = new EdDBContext())
            {
                return db.DbHelper.ExecuteDataTable(sql, namedParameters).ToList<TClientInfo>();
            }
        }
        ...
        /// <returns><c>true</c> if XXXX, <c>false</c> otherwise.</returns>
        public bool InsertEntity(TClientInfo entity)
        {
            using (EdDBContext db = new EdDBContext())
            {
                int rows = db.TClientInfos.Insert(entity);
                if (rows > 0)
                {
                    return true;
                }
        ...
        /// <param name="predicate">Lamda表达式</param>
        /// <returns><c>true</c> if XXXX, <c>false</c> otherwise.</returns>
        public bool DeleteEntity(Expression<Func<TClientInfo, bool>> predicate)
        {
            using (EdDBContext db = new EdDBContext())
            {
                TClientInfo entity = db.TClientInfos.Where(predicate).First();
                int rows = db.TClientInfos.Delete(entity);
                if (rows > 0)
                {
                    return true;
                }
        ...
        public bool DeletesEntity(List<string> predicates)
        {
            using (EdDBContext db = new EdDBContext())
            {
                ...
                try
                {
                    ...
                    {
                        var item = db.TClientInfos.Where(predicate).FirstOrDefault();
                        if (null != item)
                        {
                            db.TClientInfos.Delete(item);
                        }
                    }
                    tran.Commit();
        ...
        /// <param name="entity">实体对象</param>
        /// <returns><c>true</c> if XXXX, <c>false</c> otherwise.</returns>
        public bool UpdateEntity(TClientInfo entity)
        {
            using (EdDBContext db = new EdDBContext())
            {
                int rows = db.TClientInfos.Update(entity);
                if (rows > 0)
                {
                    return true;
                }
            ...
            string predicate, params object[] values)
        {
            using (EdDBContext db = new EdDBContext())
            {
                ...
                try
                {
                    ...
                    {
                        var entity = db.TClientInfos.Where(predicate, values).FirstOrDefault();
                        ...
                        db.TClientInfos.Update(entity);
                    }


                    tran.Commit();
                    LogHelper.Debug("TClientInfo字段批量更新成功。");
                    return true;
                }
                catch (Exception ex)
                {
                    tran.Rollback();
                    LogHelper.Error("TClientInfo字段批量更新异常：", ex);
                    return false;
                }
                finally
                {
        ...
        public bool UpdateEntityStatus(List<Field> fields, params object[] values)
        {
            using (EdDBContext db = new EdDBContext())
            {
                ...
                try
                {
                    foreach (var field in fields)
                    {
                        var entity = db.TClientInfos.Where(field.Predicate, values).FirstOrDefault();
                        ...
                        if (rows > 0)
                        {
                            ...
                            TClientMaintenance clientMaintenance = new TClientMaintenance()
                            {
                                ...
                                CMainContent = "将客户【" + entity.ClientName + "】放入回收站。",
                                CMainTime = DateTime.Now,
                                CMainCreater = entity.ClientCreater.ToString()
                            };

                            db.TClientMaintenances.Insert(clientMaintenance);
                        }
                    }
