...
namespace Ed.Dao.Impl
{
    ...
    /// <summary>
    /// 自动生成的实现ITOrderDao接口的Dao类。 2014-10-15 10:05:36 By 唐有炜
    /// </summary>
    public class TOrderDaoImpl : ITOrderDao
    {
        ...
        /// <returns>返回所有数据总数</returns>
        public int GetCount(Expression<Func<TOrder, bool>> predicate)
        {
            using (EdDBContext db = new EdDBContext())
            {
                var models = db.TOrders.Where<TOrder>(predicate);
                var sqlText = models.GetProperty("SqlText");
                LogHelper.Debug(sqlText.ToString());
                return models.Count();
            }
        ...
        /// <param name="predicate">Lamda表达式</param>
        /// <returns>返回所有数据列表</returns>
        public List<TOrder> GetList(Expression<Func<TOrder, bool>> predicate)
        {
            using (EdDBContext db = new EdDBContext())
            {
                var models = db.TOrders.Where<TOrder>(predicate);
        ...
        public TOrder GetEntity(Expression<Func<TOrder, bool>> predicate)
        {
            using (EdDBContext db = new EdDBContext())
            {
                var model = db.TOrders.Where<TOrder>(predicate);
            ...
            params object[] values)
        {
            using (EdDBContext db = new EdDBContext())
            {
                ...
                {
                    model =
                        ((IQueryable<object>)db.TOrders.Where(predicate, values).Select(selector, values));
                }
                else
                {
                    model = ((IQueryable<object>)db.TOrders.Where(predicate, values));
                }
        ...
        public IQueryable<Object> GetFields(string selector, string predicate, params object[] values)
        {
            using (EdDBContext db = new EdDBContext())
            {
                var model = db.TOrders.Where(predicate, values).Select(selector);
                var sqlText = model.GetProperty("SqlText");
                LogHelper.Debug(sqlText.ToString());
        ...
        /// </summary>
        /// <returns></returns>
        public bool ExistsEntity(Expression<Func<TOrder, bool>> predicate)
        {
            using (EdDBContext db = new EdDBContext())
            {
                bool status = db.TOrders.Any(predicate);
                return status;
            }
        }
        ...
        public bool ExistsEntity(string predicate, params object[] values)
        {
            using (EdDBContext db = new EdDBContext())
            {
                var status = db.TOrders.Where(predicate,values).Any();
                return false;
            }
        }
        ...
        //查询分页
        public IPagination<TOrder> GetListByPage(int pageIndex, int pageSize, out int rowCount,
            IDictionary<string, Ed.Entity.EdEnums.OrderEmum> orders,
            Expression<Func<TOrder, bool>> predicate)
        {
            using (EdDBContext db = new EdDBContext())
            {
                var roles = db.TOrders.Where(predicate);
            ...
            using (EdDBContext db = new EdDBContext())
            {
                ...
                foreach (var page in pages)
                {
                    ...
                    foreach (var propertyInfo in propertyInfos)
                    {
                        ...
                        {
                            case "OrderCreater":
                                int OrderCreater = int.Parse(value.ToString());
                                object objOrderCreater =
                                    db.TSysUsers.Where(u => u.Id == OrderCreater).Select(u => u.UserTname).FirstOrDefault();
                                var OrderCreaterName = "";
                                if (null != objOrderCreater)
                                {
                                    OrderCreaterName = objOrderCreater.ToString();
                                    result.Add("OrderCreaterName", OrderCreaterName);
                                }
                                break;
                            case "OrderWeihu"://维护人
                                if (null != value)
                                {
                                    int OrderWeihu = int.Parse(value.ToString());
                                    object objOrderWeihu =
                                      db.TSysUsers.Where(u => u.Id == OrderWeihu).Select(u => u.UserTname).FirstOrDefault();
                                    var OrderWeihuName = "";
                                    if (null != objOrderWeihu)
                                    {
                                        OrderWeihuName = objOrderWeihu.ToString();
                                        result.Add("OrderWeihuName", OrderWeihuName);
                                    } 
                                }
                            ...
                            case "ClientId"://
                                if (null != value)
                                {
                                    int ClientId = int.Parse(value.ToString());
                                    object objClientName=
                                      db.TClientInfos.Where(c => c.Id == ClientId).Select(c=>c.ClientName).FirstOrDefault();
                                    object objClientTel1Name =
                                      db.TClientInfos.Where(c => c.Id == ClientId).Select(c => c.ClientTel1).FirstOrDefault();
                                    var ClientName = "";
                                    var ClientTel1 = "";
                                    if (null != objClientName)
                                    {
                                        ClientName = objClientName.ToString();
                                        result.Add("ClientName", ClientName);
                                    }
                                    if (null != objClientTel1Name)
                                    {
                                        ClientTel1 = objClientTel1Name.ToString();
                                        result.Add("ClientTel1", ClientTel1);
                                    }
                                }
        ...
        public DataTable GetListBySql(string sql, dynamic namedParameters)
        {
            using (EdDBContext db = new EdDBContext())
            {
                return db.DbHelper.ExecuteDataTable(sql, namedParameters).ToList<TOrder>();
            }
        ...
        public bool InsertEntity(TOrder entity)
        {
            using (EdDBContext db = new EdDBContext())
            {
                int rows = db.TOrders.Insert(entity);
                if (rows > 0)
                {
                    return true;
                }
        ...
        /// </summary>
        /// <param name="predicate">Lamda表达式</param>
        public bool DeleteEntity(Expression<Func<TOrder, bool>> predicate)
        {
            using (EdDBContext db = new EdDBContext())
            {
                TOrder entity = db.TOrders.Where(predicate).First();
                int rows = db.TOrders.Delete(entity);
                if (rows > 0)
                {
                    return true;
                }
        ...
        public bool DeletesEntity(List<string> predicates)
        {
            using (EdDBContext db = new EdDBContext())
            {
                ...
                try
                {
                    ...
                    {
                        var item = db.TOrders.Where(predicate).FirstOrDefault();
                        if (null != item)
                        {
                            db.TOrders.Delete(item);
                        }
                    }
                    tran.Commit();
        ...
        /// </summary>
        /// <param name="entity">实体对象</param>
        public bool UpdateEntity(TOrder entity)
        {
            using (EdDBContext db = new EdDBContext())
            {
                int rows = db.TOrders.Update(entity);
                if (rows > 0)
                {
                    return true;
                }
        ...
        /// <summary>
        /// 使用LINQ批量更改TOrder状态 2014-09-05 14:58:50 By 唐有炜
        /// </summary>
             ...
             string predicate, params  object[] values)
        {
            ...
            using (EdDBContext db = new EdDBContext())
            {
                ...
                try
                {
                    ...
                    {
                        var entity = db.TOrders.Where(predicate,values).FirstOrDefault();
                        ...
                        db.TOrders.Update(entity);
                    }


                    tran.Commit();
                    LogHelper.Debug("TOrder字段批量更新成功。");
                    return true;
                }
                catch (Exception ex)
                {
                    tran.Rollback();
                    LogHelper.Error("TOrder字段批量更新异常：", ex);
                    return false;
                }
                finally
                {
        ...
        /// <summary>
        /// 使用LINQ批量更改Order状态 2014-09-05 14:58:50 By 唐有炜
        /// </summary>
        /// <param name="fields">要更新的字段（支持批量更新）</param>
        ...
        public bool UpdateEntityStatus(List<Field> fields, params object[] values)
        {
            using (EdDBContext db = new EdDBContext())
            {
                ...
                try
                {
                    foreach (var field in fields)
                    {
                        var entity = db.TOrders.Where(field.Predicate, values).FirstOrDefault();
