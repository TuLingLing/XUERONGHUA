...
namespace Ed.Dao.Impl
{
    ...
    /// <summary>
    /// 自动生成的实现ITPregnanterServiceRecordDao接口的Dao类。 2014-10-25 09:54:19 By 唐有炜
    /// </summary>
    public class TPregnanterServiceRecordDaoImpl : ITPregnanterServiceRecordDao
    {
        ...
        public int GetCount()
        {
            ...
            {
                var models = db.TPregnanterServiceRecords;
        ...
        /// <returns>返回所有数据总数</returns>
        public int GetCount(Expression<Func<TPregnanterServiceRecord, bool>> predicate)
        {
            using (EdDBContext db = new EdDBContext())
            {
                var models = db.TPregnanterServiceRecords.Where<TPregnanterServiceRecord>(predicate);
                var sqlText = models.GetProperty("SqlText");
                LogHelper.Debug(sqlText.ToString());
                return models.Count();
            }
        ...
        public List<TPregnanterServiceRecord> GetList()
        {
            using (EdDBContext db = new EdDBContext())
            {
                var models = db.TPregnanterServiceRecords;
        ...
        /// <param name="predicate">Lamda表达式</param>
        /// <returns>返回所有数据列表</returns>
        public List<TPregnanterServiceRecord> GetList(Expression<Func<TPregnanterServiceRecord, bool>> predicate)
        {
            using (EdDBContext db = new EdDBContext())
            {
                var models = db.TPregnanterServiceRecords.Where<TPregnanterServiceRecord>(predicate);
        ...
        public TPregnanterServiceRecord GetEntity(Expression<Func<TPregnanterServiceRecord, bool>> predicate)
        {
            using (EdDBContext db = new EdDBContext())
            {
                var model = db.TPregnanterServiceRecords.Where<TPregnanterServiceRecord>(predicate);
            ...
            params object[] values)
        {
            using (EdDBContext db = new EdDBContext())
            {
                ...
                {
                    model =
                        ((IQueryable<object>)db.TPregnanterServiceRecords.Where(predicate, values).Select(selector, values));
                }
                else
                {
                    model = ((IQueryable<object>)db.TPregnanterServiceRecords.Where(predicate, values));
                }
        ...
        public IQueryable<Object> GetFields(string selector, string predicate, params object[] values)
        {
            using (EdDBContext db = new EdDBContext())
            {
                var model = db.TPregnanterServiceRecords.Where(predicate, values).Select(selector);
                var sqlText = model.GetProperty("SqlText");
                LogHelper.Debug(sqlText.ToString());
        ...
        /// </summary>
        /// <returns></returns>
        public bool ExistsEntity(Expression<Func<TPregnanterServiceRecord, bool>> predicate)
        {
            using (EdDBContext db = new EdDBContext())
            {
                bool status = db.TPregnanterServiceRecords.Any(predicate);
                return status;
            }
        }
        ...
        public bool ExistsEntity(string predicate, params object[] values)
        {
            using (EdDBContext db = new EdDBContext())
            {
                bool status = db.TPregnanterServiceRecords.Where(predicate, values).Any();
                return status;
            }
        }

        //查询分页
        public IPagination<TPregnanterServiceRecord> GetListByPage(int pageIndex, int pageSize, out int rowCount,
            IDictionary<string, Ed.Entity.EdEnums.OrderEmum> orders,
            Expression<Func<TPregnanterServiceRecord, bool>> predicate)
        {
            using (EdDBContext db = new EdDBContext())
            {
                var roles = db.TPregnanterServiceRecords.Where(predicate);
                 ...
                 out int recordCount, params object[] values)
        {
            ...
            using (EdDBContext db = new EdDBContext())
            {
                //获取查询结果
                //加上扩展字段值
                var temps = db.TPregnanterServiceRecords;
                      ...
                      foreach (var propertyInfo in propertyInfos)
                        {
                            ...
                            switch (name)
                            {
                                case "PServiceCreater":
                                    int PServiceCreater = int.Parse(value.ToString());
                                    object objPServiceCreater =
                                        db.TSysUsers.Where(u => u.Id == PServiceCreater)
                                            .Select(u => u.UserTname)
                                            .FirstOrDefault();
                                    var PServiceCreaterName = "";
                                    if (null != objPServiceCreater)
                                    {
                                        PServiceCreaterName = objPServiceCreater.ToString();
                                        result.Add("PServiceCreaterName", PServiceCreaterName);
                                    }
                                    break;
                               default: //添加默认字段
                                    result.Add(name, value);
        ...
        public DataTable GetListBySql(string sql, dynamic namedParameters)
        {
            using (EdDBContext db = new EdDBContext())
            {
                return db.DbHelper.ExecuteDataTable(sql, namedParameters).ToList<TPregnanterServiceRecord>();
            }
        ...
        public bool InsertEntity(TPregnanterServiceRecord entity)
        {
            using (EdDBContext db = new EdDBContext())
            {
                int rows = db.TPregnanterServiceRecords.Insert(entity);
                if (rows > 0)
                {
                    return true;
                }
        ...
        /// </summary>
        /// <param name="predicate">Lamda表达式</param>
        public bool DeleteEntity(Expression<Func<TPregnanterServiceRecord, bool>> predicate)
        {
            using (EdDBContext db = new EdDBContext())
            {
                TPregnanterServiceRecord entity = db.TPregnanterServiceRecords.Where(predicate).First();
                int rows = db.TPregnanterServiceRecords.Delete(entity);
                if (rows > 0)
                {
                    return true;
                }
        ...
        public bool DeletesEntity(List<string> predicates)
        {
            using (EdDBContext db = new EdDBContext())
            {
                ...
                try
                {
                    ...
                    {
                        var item = db.TPregnanterServiceRecords.Where(predicate).FirstOrDefault();
                        if (null != item)
                        {
                            db.TPregnanterServiceRecords.Delete(item);
                        }
                    }
                    tran.Commit();
        ...
        /// </summary>
        /// <param name="entity">实体对象</param>
        public bool UpdateEntity(TPregnanterServiceRecord entity)
        {
            using (EdDBContext db = new EdDBContext())
            {
                int rows = db.TPregnanterServiceRecords.Update(entity);
                if (rows > 0)
                {
                    return true;
                }
        ...
        /// <summary>
        /// 使用LINQ批量更改TServiceInfo状态 2014-09-05 14:58:50 By 唐有炜
        /// </summary>
        ...
        public bool UpdateEntityStatus(List<Field> fields, params  object[] values)
        {
            ...
            using (EdDBContext db = new EdDBContext())
            {
                ...
                try
                {
                    ...
                    {
                        var entity = db.TPregnanterServiceRecords.Where(field.Predicate, values).FirstOrDefault();
                        ...
                        db.TPregnanterServiceRecords.Update(entity);
                    }


                    tran.Commit();
                    LogHelper.Debug("TPregnanterServiceRecord字段批量更新成功。");
                    return true;
                }
                catch (Exception ex)
                {
                    tran.Rollback();
                    LogHelper.Error("TPregnanterServiceRecord字段批量更新异常：", ex);
                    return false;
                }
                finally
                {
