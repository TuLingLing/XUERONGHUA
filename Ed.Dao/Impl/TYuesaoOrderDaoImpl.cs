...
namespace  Ed.Dao.Impl
{
    ...
    /// <summary>
    /// 自动生成的实现ITYuesaoOrderDao接口的Dao类。 2014-10-15 10:05:36 By 唐有炜
    /// </summary>
 public class TYuesaoOrderDaoImpl:ITYuesaoOrderDao
    {
	  ...
	  #region 读操作
	   ...
	   /// <summary>
        ...
        /// <returns>返回所有数据总数</returns>
       public int GetCount(Expression<Func<TYuesaoOrder, bool>> predicate)
        {
             using (EdDBContext db=new EdDBContext())
            {
             var models= db.TYuesaoOrders.Where<TYuesaoOrder>(predicate);
			 var sqlText = models.GetProperty("SqlText");
             LogHelper.Debug(sqlText.ToString());
			 return models.Count();
            }
        ...
        public List<TYuesaoOrder> GetList() 
        {
          using (EdDBContext db=new EdDBContext())
            {
             var models= db.TYuesaoOrders;
        ...
        /// <param name="predicate">Lamda表达式</param>
        /// <returns>返回所有数据列表</returns>
       public List<TYuesaoOrder> GetList(Expression<Func<TYuesaoOrder, bool>> predicate)
        {
             using (EdDBContext db=new EdDBContext())
            {
             var models= db.TYuesaoOrders.Where<TYuesaoOrder>(predicate);
        ...
        public TYuesaoOrder GetEntity(Expression<Func<TYuesaoOrder, bool>> predicate) 
        {
            using (EdDBContext db=new EdDBContext())
            {
                var model =db.TYuesaoOrders.Where<TYuesaoOrder>(predicate);
        ...
        public Dictionary<string, object> GetEntityWithExp(string selector, string expFields, string expSelector,
		    ...
		    using (EdDBContext db=new EdDBContext())
            {
                ...
                {
                    model =
                        ((IQueryable<object>) db.TYuesaoOrders.Where(predicate, values).Select(selector, values));
                }
                else
                {
                    model = ((IQueryable<object>)db.TYuesaoOrders.Where(predicate, values));
                }
        ...
        public IQueryable<Object> GetFields(string selector, string predicate, params object[] values)
        {
            using (EdDBContext db = new EdDBContext())
            {
                var model = db.TYuesaoOrders.Where(predicate, values).Select(selector);
                var sqlText = model.GetProperty("SqlText");
                LogHelper.Debug(sqlText.ToString());
        ...
        /// </summary>
        /// <returns></returns>
       public   bool ExistsEntity(Expression<Func<TYuesaoOrder , bool>> predicate)
	   {
            using (EdDBContext db=new EdDBContext())
            {
               bool status= db.TYuesaoOrders.Any(predicate);
               return status;
            }
        }
       ...
       public bool ExistsEntity(string predicate, params object[] values)
       {
           using (EdDBContext db = new EdDBContext())
           {
               bool status = db.TYuesaoOrders.Where(predicate,values).Any();
               return status;
           }
       }
	   ...
	   //查询分页
        public IPagination<TYuesaoOrder> GetListByPage(int pageIndex, int pageSize, out int rowCount,
            IDictionary<string, Ed.Entity.EdEnums.OrderEmum> orders,
            Expression<Func<TYuesaoOrder, bool>> predicate)
        {
            using (EdDBContext db = new EdDBContext())
            {
                var roles = db.TYuesaoOrders.Where(predicate);
	  ...
	  //===========================================================================
	   /// <summary>
        ...
        public DataTable GetListBySql(string sql, dynamic namedParameters)
        {
          using (EdDBContext db=new EdDBContext())
            {
               return db.DbHelper.ExecuteDataTable(sql,namedParameters).ToList<TYuesaoOrder>();
            }
   ...
   #region 写操作
        ...
        public bool InsertEntity(TYuesaoOrder entity)
        {
            using (EdDBContext db=new EdDBContext())
            {
              int rows=  db.TYuesaoOrders.Insert(entity);
				 if (rows > 0)
                {
                    return true;
                }
       ...
       /// <summary>
        ...
        /// </summary>
         /// <param name="predicate">Lamda表达式</param>
        public bool DeleteEntity(Expression<Func<TYuesaoOrder , bool>> predicate) 
        {
            using (EdDBContext db=new EdDBContext())
            {
                TYuesaoOrder  entity = db.TYuesaoOrders.Where(predicate).First();
                int rows=db.TYuesaoOrders.Delete(entity);
				 if (rows > 0)
                {
                    return true;
                }
        ...
        public bool DeletesEntity(List<string> predicates)
        {
            using (EdDBContext db=new EdDBContext())
            {
                ...
                try
                {
                     ...
                     {
                        var item = db.TYuesaoOrders.Where(predicate).FirstOrDefault();
                        if (null != item)
                         {
                            db.TYuesaoOrders.Delete(item);
                         }
                    }
                    tran.Commit();
        ...
        /// </summary>
        /// <param name="entity">实体对象</param>
        public bool UpdateEntity(TYuesaoOrder entity)
        {
            using (EdDBContext db=new EdDBContext())
            {
               int rows= db.TYuesaoOrders.Update(entity);
			   if (rows > 0)
                {
                    return true;
                }
        ...
        /// <summary>
        /// 使用LINQ批量更改TYuesaoOrder状态 2014-09-05 14:58:50 By 唐有炜
        /// </summary>
       ...
       public bool UpdateEntityStatus(Dictionary<string, object> fields,
		 ...
		 using (EdDBContext db=new EdDBContext())
            {
                ...
                try
                {
                    ...
                    {
                     var entity = db.TYuesaoOrders.Where(predicate,values).FirstOrDefault();
                        ...
                        db.TYuesaoOrders.Update(entity);
                    }


                    tran.Commit();
                    LogHelper.Debug("TYuesaoOrder字段批量更新成功。");
                    return true;
                }
                catch (Exception ex)
                {
                    tran.Rollback();
                    LogHelper.Error("TYuesaoOrder字段批量更新异常：", ex);
                    return false;
                }
                finally
                {
        ...
        /// </summary>
        /// <param name="entity">实体对象</param>
        public bool InsertEntityTran(TOrder order, TYuesaoOrder yuesaoOrder, TOrderFinance orderFinance)
        {
            using (EdDBContext db = new EdDBContext())
            {
                ...
                try
                {
                    ...
                    db.TYuesaoOrders.Insert(yuesaoOrder);
                    tran.Commit();
                    LogHelper.Debug("月嫂订单添加成功！");
                    return true;
                }
                catch (Exception ex)
                {
                    tran.Rollback();
                    LogHelper.Error("月嫂订单添加异常：", ex);
                    return false;
                }
                finally
                {
        ...
        /// <summary>
        /// 修改实体(事务控制先添加订单表，在添加月嫂订单表)
        /// </summary>
        /// <param name="entity">实体对象</param>
        public bool UpdateEntityTran(TOrder order, TYuesaoOrder yuesaoOrder)
        {
            using (EdDBContext db = new EdDBContext())
            {
                ...
                try
                {
                    ...
                    db.TYuesaoOrders.Update(yuesaoOrder);
                    tran.Commit();
                    LogHelper.Debug("月嫂订单确认成功！");
                    return true;
                }
                catch (Exception ex)
                {
                    tran.Rollback();
                    LogHelper.Error("月嫂订单确认异常：", ex);
                    return false;
                }
                finally
                {
        ...
        /// 批量删除（实体集合）
        /// </summary>
        /// <param name="ids">这里是订单编号集合</param>
        /// <returns><c>true</c> if XXXX, <c>false</c> otherwise.</returns>
        public bool DeleteEntityTran(string ids)
        {
            using (EdDBContext db = new EdDBContext())
            {
                ...
                try
                {
                    ...
                    foreach (var orderCodeObj in idArray)
                    {
                          var orderCode = orderCodeObj.ToString();
                        db.TOrders.Delete(o => o.OrderCode == orderCode);
                        db.TYuesaoOrders.Delete(o => o.OrderCode == orderCode);
                        db.TOrderFinances.Delete(o => o.OrderCode == orderCode);
                    }
                    tran.Commit();
                    LogHelper.Debug("订单批量删除成功！");
                    return true;
                }
                catch (Exception ex)
                {
                    tran.Rollback();
                    LogHelper.Error("订单批量删除异常：", ex);
                    return false;
                }
                finally
                {
