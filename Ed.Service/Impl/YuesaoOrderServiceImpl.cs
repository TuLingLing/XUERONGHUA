...
public  class YuesaoOrderServiceImpl: IYuesaoOrderService
	...
	try
            {
                var fields =  YuesaoOrderDao.GetFields(selector, predicate,values);
                return fields;
            }
            catch (Exception ex)
            {
	  ...
	  #region 添加、修改
        ...
        public  bool AddYuesaoOrder(TYuesaoOrder YuesaoOrder)
		{
		 var status = false;
		       try
                ...
                {
                    LogHelper.Error("YuesaoOrdert添加失败。", ex);
                    status = false;
                }
        ...
        public bool AddYuesaoOrderTran(TOrder order, TYuesaoOrder yuesaoOrder, TOrderFinance orderFinance)
        {

            var status = false;
            try
            {
                status = YuesaoOrderDao.InsertEntityTran(order, yuesaoOrder,orderFinance);
            }
            catch (Exception ex)
            {
    ...
    /// </summary>
    /// <param name="entity">实体对象</param>
    public bool EditYuesaoOrderTran(TOrder order, TYuesaoOrder yuesaoOrder)
    {
        var status = false;
        try
        {
            status = YuesaoOrderDao.UpdateEntityTran(order, yuesaoOrder);
        }
        catch (Exception ex)
        {
            LogHelper.Error("YuesaoOrder确认失败。", ex);
            status = false;
        }
...
//                return false;
//            }
//            return  YuesaoOrderDao.UpdateEntityStatus(fields, predicate);
//			}
		 #endregion
        ...
        /// <param name="ids">ids</param>
        /// <returns>删除状态</returns>
        public bool DeleteYuesaoOrderTran(string ids)
        {
            var status = false;
            try
            {
             
                status = YuesaoOrderDao.DeleteEntityTran(ids);
            }
            catch (Exception ex)
            {
                LogHelper.Error("获取YuesaoOrder失败。", ex);
